...
client.setConfig({apiKey: "enter-key",  server: "enter-server",});
...
app.post("/", function (req, res){
    ...
    const run = async () => {
        const response = await client.lists.addListMember("enter-list", {
            email_address: email,
            status: "subscribed",
            merge_fields: {
                FNAME: firstName,
...
app.listen(process.env.PORT || 5000, function (){ //process.env.PORT for heroku
    setTimeout(() => {
        spinner.color = 'blue';
